...
class Frontend {
    ...
    public function __construct() {
        ...
        if (!$survey_id) {
            return '<div class="surveyflow-msg">' . esc_html__('No survey specified.', 'surveyflow') . '</div>';
        }
    ...
    private function enqueue_assets() {
        wp_enqueue_style(
            ...
            SURVEYFLOW_VERSION
                        ...
                        $attr_pairs = [];
                        $attr_pairs[] = sprintf(' data-type="%s"', esc_attr($is_preq ? 'prequalifier' : $type));
                        if ($is_preq) {
                            $attr_pairs[] = sprintf(' data-subtype="%s"', esc_attr($q['subtype'] ?? 'radio'));
                        }

                        echo '<div class="surveyflow-question"' . implode('', $attr_pairs) . ' data-index="' . esc_attr($i) . '">';
                                        $inputType = ($type === 'checkbox') ? 'checkbox' : 'radio';
                                        $other = !empty($opt['other']);
                                        echo '<label class="sf-option">';
                                        echo '<input type="'.esc_attr($inputType).'" name="'.esc_attr($name).($type==='checkbox'?'[]':'').'" value="'.esc_attr($opt['label']).'" id="'.esc_attr($oid).'"'.($other?' data-other="1"':'').'> ';
                                echo '<select name="'.esc_attr($name).'" class="sf-input">';
                                echo '<option value="">-- '.esc_html__('Select','surveyflow').' --</option>';
                                            $inputType = ($sub === 'checkbox') ? 'checkbox' : 'radio';
                                            echo '<label class="sf-option">';
                                            echo '<input type="'.esc_attr($inputType).'" data-disqualify="'.($disq?1:0).'" name="'.esc_attr($name).($sub==='checkbox'?'[]':'').'" value="'.esc_attr($opt['label']).'" id="'.esc_attr($oid).'"> ';
    ...
    public function handle_submit() {
        ...
        foreach ($media_keys as $mk) {
            if (!isset($_FILES[$mk]) || empty($_FILES[$mk]['name'])) {
                continue;
            }
            ...
            if (!empty($uploaded['error'])) {
                /* translators: %s: error message returned from the upload handler. */
